...
def main():
    ...
    bizphoto_id = "'1ocB2s3Y4S8ZaQfbQfXvsaA2pJcNi-Lam'"  # Enter The Downloadable folder ID From Shared Link

    results = service.files().list(
        pageSize=1000, q=bizphoto_id+" in parents", fields="nextPageToken, files(id, name, mimeType)").execute()
    ...
    if not items:
        ...
        for item in items:
            if item['mimeType'] == 'application/vnd.google-apps.folder':
                if not os.path.isdir("BizPhotos"):
                    os.mkdir("BizPhotos")
                bfolderpath = os.getcwd()+"/BizPhotos/"
